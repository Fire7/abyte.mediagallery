...
abyteMG.resizeImageBox = function (img, mgModal, windowWidth) {
    ...
    var imgW = img[0].naturalWidth || img[0].width,
        imgH = img[0].naturalHeight || img[0].height,
        imgK = imgW / imgH,
        windowW = document.documentElement.clientWidth + (windowWidth || 0),
        windowH =  document.documentElement.clientHeight - 80,
        windowK = windowW / windowH;


    if (imgK > windowK || (imgK < 1 && windowW - imgW < 200)) {
        mgModal.css({
            'width' : '100%',
            'height': 'auto'
        });
        ...
        img.css({
            'height' : (mgModal.height() - 80) + 'px'/* 'calc(100% - 80px)'*/,
            'width' : 'auto'
        });
    }
}
                    ...
                    complete: function () {
                        ...
                        if (img.length)
                            img[0].onload = function () {
                                if ( img[0].naturalWidth> maxImageWidth || img[0].naturalHeight > maxImageHeight)
